        ...
        public bool UploadPX4(string filename, BoardDetect.boards board)
        {
            ...
            while (DateTime.Now < DEADLINE)
            {
                ...
                foreach (string port in allports)
                {
                    ...
                    {
                        log.Error(ex);
                        CustomMessageBox.Show("与板子通信断开！", "通信丢失");
                        up.close();
                        return false;
                    }
                    catch
...
//VR boards have no tone alarm
        ...
        {
            if (board == BoardDetect.boards.viga_v1|board == BoardDetect.boards.px4 || board == BoardDetect.boards.px4v2 || board == BoardDetect.boards.px4v4)
            {
                try
                {
        ...
        byte[] readIntelHEXv2(StreamReader sr)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                if (line.StartsWith(":"))
                {
                    ...
                    {
                        CustomMessageBox.Show("当前加载的hex文件无效，请重试!");
                        throw new Exception("校验错误-hex文件无效");
                    }
                }
            ...
            if (!hitend)
            {
                CustomMessageBox.Show("当前hex文件未含有结束标志,操作无效!");
                throw new Exception("文件结尾没有有效结束标志");
            }
