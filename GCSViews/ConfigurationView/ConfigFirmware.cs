...
namespace MissionPlanner.GCSViews.ConfigurationView
{
    partial class ConfigFirmware : MyUserControl, IActivate
    {
        ...
        private void updateDisplayName(Firmware.software temp)
        {
            ...
            //}
            //else
            //if (temp.url2560.ToLower().Contains("ac2-quad-".ToLower()) ||
        ...
        {
            using (var fd = new OpenFileDialog {Filter = "Firmware (*.hex;*.vi)|*.hex;*.vi|All files (*.*)|*.*" })
            {
                ...
                if (File.Exists(fd.FileName))
                {
                    ...
                    try
                    {
                        if (fd.FileName.ToLower().EndsWith(".px4")| fd.FileName.ToLower().EndsWith(".vi"))
                            boardtype = BoardDetect.boards.viga_v1;
                        else 
