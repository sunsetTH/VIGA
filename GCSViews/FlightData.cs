...
//            Gheading.Location = new Point(Galt.Right, 0);
        ...
        public bool screencurrentstatus=true;
        ...
        private void BUT_resumemis_Click_1(object sender, EventArgs e)
        {
        ...
        private void pictureBox1_Click(object sender, EventArgs e)
        {
            ...
            foreach (MainSwitcher.Screen sc in MainV2.View.screens)
            {
                if (sc.Name == "FlightPlanner")
                {
                    if (screencurrentstatus == true)
                    {
                        screencurrentstatus = false;
                        ...
                        CHK_autopan.Visible = false;
                        coords1.Visible = false;
                        //CB_tuning.Visible = false;
                        //but.BringToFront();
                    }
                    else
                    {
                        if (screencurrentstatus == false)
                        {
                            screencurrentstatus = true;
                            splitContainer1.Panel2.Controls.Remove(sc.Control);
                            splitContainer1.Panel2.Controls.Remove((Control)sender);
                            sc.Control.Visible = false;
                            ...
                            CHK_autopan.Visible = true;
                            coords1.Visible = true;
                            //CB_tuning.Visible = true;
                        }
                    }
                    break;
                }
        ...
        private void myButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("请确保系统连接正常!", Strings.ERROR);
             }
        ...
        private void onekeyfly_Click(object sender, EventArgs e)
        {
            if (MainV2.comPort.BaseStream.IsOpen)
            {
                MainV2.comPort.setMode("Stabilize");
                ...
                if (Settings.Instance.ContainsKey("guided_alt"))
                 alt = Settings.Instance["guided_alt"];
                alt = onekeyfly.Value.ToString();
                Settings.Instance["guided_alt"] = alt;

                int intalt = (int)(100 * CurrentState.multiplierdist);
                if (!int.TryParse(alt, out intalt))
                {
                    CustomMessageBox.Show("高度值异常！");
                    return;
                }
                ...
                if (MainV2.comPort.doARM(true))
                {
                    ...
                    CustomMessageBox.Show(Strings.CommandFailed+" 请仔细检查起飞检查单中项目！", Strings.ERROR);
