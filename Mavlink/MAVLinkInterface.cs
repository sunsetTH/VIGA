    ...
    public class MAVLinkInterface : MAVLink, IDisposable
    {
        ...
        public void Open(bool getparams, bool skipconnectedcheck = false)
        {
            ...
            frmProgressReporter = new ProgressReporterDialogue
            {
                ...
                Text = "数据连接"// Strings.MavlinkConnecting
            };
...
4. Try a diffrent usb port\n\n" +
        ...
        */
        ...
        private Hashtable getParamListBG()
        {
                    ...
                    if (buffer.msgid == (byte)MAVLINK_MSG_ID.PARAM_VALUE && buffer.sysid == req.target_system && buffer.compid == req.target_component)
                    {
                        ...
                        if (indexsreceived.Contains(par.param_index))
                        {
                            ...
                            this.frmProgressReporter.UpdateProgressAndStatus((indexsreceived.Count*100)/param_total,
                                "系统初始化，请稍后" + Convert.ToString((indexsreceived.Count * 100) / param_total)+"%");
                            continue;
                        }
            ...
            {
                var exp = new Exception("丢失参数" + indexsreceived.Count + " vs " + param_total);
                frmProgressReporter.doWorkArgs.ErrorMessage = exp.Message;
                throw exp;
            }
            giveComport = false;
            ...
            try
            {
                if ((message.header == 'U' || message.header == 0xfE|| message.header == 0xfd) && buffer.Length >= message.payloadlength)
                {
                    ...
                    if (msgid == (byte)MAVLink.MAVLINK_MSG_ID.STATUSTEXT) // status text
                    {
                        ...
                        if (printit)
                        {   if (logdata.StartsWith("GPS") && logdata.Contains("GSOF"))
                            {
                                MAVlist[sysid, compid].cs.messageHigh = "    系统GPS协议为GSOF";
                                MAVlist[sysid, compid].cs.messageHighTime = DateTime.Now;

                            }
                            else if (logdata.StartsWith("PreArm") && logdata.Contains("Compass not calibrated"))
                            {
                                logdata = "磁罗盘未校准";
                                MAVlist[sysid, compid].cs.messageHigh = "      磁罗盘未校准";
                                MAVlist[sysid, compid].cs.messageHighTime = DateTime.Now;

                            }
                            else if (logdata.StartsWith("PreArm:") && logdata.Contains("Accel"))
                            {
                                logdata = "加表未校准";
                                MAVlist[sysid, compid].cs.messageHigh = "      加表未校准";
                                MAVlist[sysid, compid].cs.messageHighTime = DateTime.Now;

                            }
                            else if(logdata.StartsWith("PreArm:") && logdata.Contains("RC"))
                            {
                            ...
                            else if (logdata.ToUpper().Contains("MODE") && logdata.ToLower().Contains("stablized"))
                            {
                                
                                logdata = "飞行模式更改Stablized";
                                MAVlist[sysid, compid].cs.messageHigh = logdata;
                                MAVlist[sysid, compid].cs.messageHighTime = DateTime.Now;
                            }
                            if (logdata.ToUpper().Contains("MODE") && logdata.ToLower().Contains("auto"))
                            {
                            ...
                            if (logdata.ToUpper().Contains("MODE") && logdata.ToLower().Contains("althold"))
                            {
                            ...
                            if (logdata.ToUpper().Contains("MODE") && logdata.ToLower().Contains("poshold"))
                            {
                            ...
                            else if (logdata.Contains("PreArm: Need 3D Fix"))
                            {
                            ...
                            if (logdata.Contains("Failsafe: Terrain data missing"))
                            {
